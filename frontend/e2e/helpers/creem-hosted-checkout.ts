...
export function paymentInstrument(value: unknown): HostedPaymentInstrument {
  ...
  if (
    ...
    throw new Error('Creem payment instrument format is invalid');
  }
  return result;
}
...
async function fillIfVisible(page: Page, selectors: readonly string[], value: string): Promise<void> {
  try {
