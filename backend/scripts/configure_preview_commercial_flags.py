#!/usr/bin/env python3
"""Enable all bounded Preview Commercial capabilities for two verified identities."""
...
async def enable_commercial_cohort(args: argparse.Namespace) -> dict[str, object]:
    ...
    try:
        async with session_factory() as db:
            async with db.begin():
                ...
                if (
                    ...
                    raise ValueError(
                        "Preview activation coordinates are not eligible for Commercial acceptance"
                    )
                if (
                    activation.reservation_expires_at is None
                    or expires_at > activation.reservation_expires_at
                ):
                    raise ValueError(
                        "Preview Commercial cohort exceeds the activation reservation"
                    )
                ...
                for flag in flags:
                    if (
                        ...
                        raise ValueError(
                            "all Preview capability flags must be at the OFF baseline"
                        )
                ...
                if len(bindings) != 2:
                    raise ValueError(
                        "exactly two unconsumed deployment-bound Google identities are required"
                    )
                identity_hashes = tuple(
                    sorted(binding.subject_hmac for binding in bindings)
                )
                decisions = {}
                for capability in COMMERCIAL_CAPABILITIES:
        ...
        return {
            ...
            "capabilities": sorted(
                capability.value for capability in COMMERCIAL_CAPABILITIES
            ),
            "identity_count": 2,
...
def main() -> int:
    ...
    try:
        result = asyncio.run(enable_commercial_cohort(args))
