...
class PasswordAuthTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_register_hashes_password_and_returns_session(self) -> None:
        ...
        response = await auth_router.register(
            RegisterRequest(username="BrideUser", password="secret123"),
            _request("10.0.0.1"),
            db,
        )
    ...
    async def test_password_login_uses_existing_user_identity(self) -> None:
        ...
        registered = await auth_router.register(
            RegisterRequest(username="creator01", password="secret123"),
            _request("10.0.0.2"),
            db,
        )
    ...
    async def test_duplicate_username_is_rejected(self) -> None:
        db = _FakeDb()
        await auth_router.register(RegisterRequest(username="same_name", password="secret123"), _request("10.0.0.4"), db)

        with self.assertRaises(HTTPException) as raised:
            await auth_router.register(RegisterRequest(username="same_name", password="another123"), _request("10.0.0.5"), db)

        self.assertEqual(raised.exception.status_code, 409)

    async def test_wrong_password_is_rejected(self) -> None:
        db = _FakeDb()
        await auth_router.register(RegisterRequest(username="safeuser", password="secret123"), _request("10.0.0.6"), db)
