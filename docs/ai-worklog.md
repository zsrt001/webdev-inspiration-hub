...
- PR #92 merged as main `4ad7e96f4eaff8db9417229e1914bc733e149756`
  after all required checks passed. Protected run `29757239569` passed the
  private Blob read-write preflight, completed the unaliased Vercel Production
  build, and deleted the exact deployment, but GitHub could not find the object
  written from the Vercel build. The delivery cleanup proof is `NOT_CREATED`.
  This confirms that sharing the long-lived token text across the two execution
  environments still does not establish a single storage identity; the
  database rearm and role proof were not the failing boundary.
- A disposable non-secret deployment proved that Vercel's deployment-files API
  exposes uploaded source files, not files generated by the build, and was
  deleted with a confirmed not-found result. A separate local OIDC probe proved
  that the linked CLI session issues a `development` environment claim, which
  the private Blob store rejects. Both paths were therefore rejected instead
  of receiving further variants, and their exact local probe files were
  removed.
- The replacement keeps the existing GitHub read-write round-trip preflight,
  then uses pinned `@vercel/blob` 2.4.0 inside the protected job to issue one
  exact-path, 15-minute delegation for `put`, `get`, and `delete`. Vercel
  receives only the presigned create-once PUT URL; GitHub retains the GET and
  DELETE URLs, validates their store/path/operation constraints, masks them,
  excludes them from artifacts, and removes the capability files after cleanup.
  The build validates the PUT URL before any database mutation, rejects
  redirects, limits the ciphertext envelope to 10,000 bytes, and accepts the
  upload only after the returned private store and object key match exactly.
- The updated credential, database-login, CI release-contract, and Production
  release-workflow regression passes 193 tests with two explicit local
  environment/platform skips. All ten workflow shell blocks pass `bash -n`;
  Python and Node syntax checks, workflow YAML parsing, `npm ci --ignore-scripts`,
  the pure presigned-URL validation checks, and `git diff --check` pass. A live
