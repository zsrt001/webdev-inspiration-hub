...
- [ ] **Step 5: Keep refund initiation support-only and hard-gate Provider confirmation**

Add code-versioned provider fact `CREEM_DASHBOARD_REFUND_CONFIRMATION=UNVERIFIED` in `provider_contracts.py`; it is not an environment variable, frontend value, or mutable Admin flag. Creem's complete public API reference does not expose a refund-creation mutation, so `POST /payments/{purchase_id}/refund` remains a compatibility surface that returns structured 409 `refund_requires_support`, writes no refund success fact, and leaves signed `refund.created` processing active. A later evidence-only change may set VERIFIED only while binding the official Dashboard refund workflow, signed webhook schema, transaction-read confirmation, exact test purchase, local reversal, and replay-dedupe evidence to the release manifest. Do not invent or call an undocumented refund endpoint.
...
Expected: all unit/PostgreSQL tests PASS; self-service refund initiation remains intentionally unavailable while signed Dashboard-initiated refund handling passes.
...
No public subscription refund endpoint exists. A service-authorized, audited operation may process only a specific invoice's full refund after verified Creem Dashboard initiation, signed webhook, and transaction-read confirmation are available. A signed full refund appends one `SUBSCRIPTION_REVERSAL`, creates debt if consumed, and revokes linked entitlement. Partial refund opens reconciliation and freezes lineage. Invoice dispute freeze/win/loss shares the same cumulative reversal cap. Refund never implicitly cancels renewal.
...
**Stage 3 exit:** Empty-database migration reaches `0018`; real PostgreSQL concurrency proves no overspend or duplicate grants; Creem test-mode facts prove catalog, checkout, signed replay, lifecycle, reversals, disputes, and cancellation where the provider exposes a verified contract. Missing Dashboard refund confirmation or a stable subscription transaction contract keeps the affected checkout/refund capability OFF and the corresponding external gate NOT_RUN.
...
- Create: `scripts/release/collect_production_acceptance_input.py`
- Create: `scripts/release/quality_review_handoff.py`
...
- Create: `docs/operations/production-quality-review.md`
...
-> consume exact-source VERIFIED Evolink reconciliation evidence -> user delete -> object 404/410
```
...
Creem test mode additionally proves renewal, past-due recovery, duplicate/out-of-order events, partial anomaly, and dispute outcome. Production does not manufacture a real chargeback. Before either money chain, the auth phase proves refresh/logout and every legacy/header impersonation denial. A separate isolated Preview/Sandbox Evolink unknown-state phase uses one real request, discards only the caller-side response, and must prove the same Provider task/capture through signed callback, reconciliation, and database facts. Production consumes the exact-source VERIFIED contract evidence and never injects an egress fault. Six quality cases all reach READY within one initial plus at most two repair candidates and receive per-candidate human review under the fixed rubric.
...
npm --prefix frontend run test:e2e -- e2e/main-flow.spec.ts
git diff --check
git add scripts/release/run_linked_commercial_acceptance.mjs scripts/release/run_subscription_acceptance.mjs scripts/release/run_quality_acceptance.mjs scripts/release/observe_release.py scripts/release/register_bundle.py scripts/release/resolve_release_coordinates.py scripts/release/apply_activation_plan.py scripts/release/activate_with_canaries.py scripts/release/cleanup_acceptance_bindings.py scripts/release/configure_staged_auth_origin.py scripts/release/verify_provider_grant_fetch.py scripts/release/verify_safe_baseline_bridge.py scripts/release/replay_migration_window_events.py frontend/e2e/main-flow.spec.ts frontend/e2e/production-canary.spec.ts backend/tests/test_production_acceptance_contract.py backend/tests/test_production_release_workflow.py backend/tests/test_release_coordinate_resolver.py backend/tests/integration/test_safe_baseline_schema_0020_bridge.py .github/workflows/production-release.yml .github/workflows/release-observation.yml .github/workflows/integration.yml docs/ai-worklog.md
...
Extend the Stage-5 `release/provider-contracts.json` authority without changing its schema or resetting Evolink. The task refuses to proceed unless the committed Evolink signed-callback recovery status/evidence is already VERIFIED and its activation parent/diff contract validates; Creem Dashboard refund confirmation and stable subscription transaction/cancel facts remain `UNVERIFIED` until Step 6. `provider_contracts.py` continues to reject environment-variable overrides. Add only the Creem verification support needed to bind the official workflow URL/version, signed webhook and transaction-read schemas, authentication scheme, correlation semantics, error/retry taxonomy, and expected test-mode report schema. No Production catalog import occurs here.
...
Run only from the clean committed verification-support SHA. The Creem evidence must prove one exact test purchase, an authorized Dashboard refund initiation, a valid signed `refund.created` event, a matching transaction-read refund state, one local reversal, and replay dedupe; one subscription first paid transaction plus renewal with stable transaction IDs and both invoice uniqueness keys; and Creem-confirmed period-end cancellation. The Evolink evidence deliberately loses the submit response and recovers the same task through the documented attempt-bound callback without a second generation or capture. `activate_provider_contracts.py` accepts only signed, fresh, schema-valid reports from that SHA and writes nonempty evidence hashes into the versioned config. A Dashboard screenshot or self-authored fixture alone is insufficient: the signed webhook, transaction query, and database facts must agree.
...
Expected only after genuine external evidence: all three integration contracts PASS and the second commit changes only reviewed contract/evidence hashes and factual worklog status. Missing Creem Dashboard refund confirmation, stable subscription transaction/cancel evidence, or EvoLink signed-callback recovery leaves the fact UNVERIFIED, skips the activation commit, keeps the affected flag OFF, and ends 7a as NOT_RUN. Production catalog import remains later and is bound to the final SHA.

- [ ] **Step 7: Freeze the final source SHA and pass committed Preview/CI gates**

Before freeze, write and commit the user-approved overseas Worker-host addendum plus `release/worker-host-contract.json` with exact allowlisted executable/arguments for `build-push`, digest resolution, suspended deploy, `start`, `stop`, `set-dispatch`, `heartbeat`, `ensure-running`, `rollback`, `reconcile-failure`, `verify-no-release`, secret injection, and logs. `run_approved_worker_host.py` validates the contract hash, invokes argument arrays without a shell, rejects mutable tags/unlisted actions, redacts secrets, and emits canonical signed reports; the protected workflow calls only this adapter. Standard managed Worker platforms do not expose a safe host-level primitive that forwards one TLS request while discarding only its response. The genuine lost-response proof therefore remains in Step 6's isolated Preview/Sandbox contract: a real Evolink request is accepted, the client response is deliberately discarded at the test transport boundary, the signed callback binds the same task, and database evidence proves one submit/one capture. Production consumes that source-bound VERIFIED contract and never injects a network fault into the live Worker. Until a host-specific deployment contract is reviewed, the script exits NOT_RUN/nonzero. Confirm monitored support, legal-review status, authorized six-case inputs, cost/currency caps, Private Blob store, and Production approvals exist. Then require a clean tree including untracked files, approved main HEAD, all mandatory CI, and protected Preview E2E for that exact commit. No later code/config/workflow/provider commit is allowed under the same bundle; a change restarts Step 7.
...
For each authorized acceptance subject, create a deployment-bound subject-HMAC binding and set only `GOOGLE_AUTH` to cohort for `STAGED_TARGET_ID`; no canonical user ID exists yet and no non-auth capability accepts the binding. Run the real staged Google first login, atomically consume the binding, capture the resulting canonical user ID in a signed sanitized report, rotate refresh, logout, prove the revoked session plus legacy/header impersonation are denied, then perform a second real login. Only then add that canonical ID to upload, generation, checkout, subscription, private-download, and Partner-Invite cohorts. The same-runtime private-compatible baseline must already be formal and its token-only Provider grant route must pass a real Evolink fetch; Generation cohort cannot precede that proof. Clean every expired/unused binding. Run the two linked commercial chains, the debt-offset continuation, six quality cases, cross-user denial, account export/deletion, and production canary against the still-unbound staged target. `TARGET_ACCEPTED` additionally hashes the exact source-bound `EVOLINK_SUBMISSION_RECONCILIATION` contract proved in Preview/Sandbox; Production does not repeat an artificial response-loss fault. The subscription chain binds stable paid transaction ID, both invoice uniqueness keys, cancel intent/provider confirmation, refund/reversal, and entitlement IDs; otherwise `SUBSCRIPTION_BILLING` remains OFF/NOT_RUN. After all staged gates, remove the temporary Supabase redirect and staged-origin activation entry, read back/prove no wildcard or residue, and keep only the exact formal callback before target Promote.

The real lost-response case runs only in the protected Preview/Sandbox integration before source freeze. That harness uses the production submission service and a real Evolink request, discards the successful client response after the Provider accepts it, then requires the signed callback to bind the same task and the database to prove one submit, one capture, and one completed reconciliation. The resulting evidence hash is committed into `release/provider-contracts.json` for the exact tested source SHA. Missing, stale, wrong-SHA, unsigned, duplicate-submit, or callback-free evidence keeps the contract UNVERIFIED and blocks Production before any release action.
...
python scripts/release/collect_production_acceptance_input.py --phase commercial-finalize-delete --browser-report "$evidence/account-finalize-browser.json" --commercial-report "$evidence/commercial-before-delete.json" --provider-contract release/provider-contracts.json --storage-absence-report "$evidence/account-media-absence.json" --database-url-env PRODUCTION_MIGRATION_DATABASE_URL --output "$evidence/commercial-finalize-input.json"
node scripts/release/run_linked_commercial_acceptance.mjs --phase commercial-finalize-delete --base-url $stagedTargetUrl --commercial-report "$evidence/commercial-before-delete.json" --output "$evidence/commercial-chain.json"
...
python scripts/release/register_bundle.py advance --kind COMMERCIAL_7A --expected-phase WORKER_DISPATCH_ENABLED --phase TARGET_ACCEPTED --auth-security-report "$evidence/first-login-users.json" --staged-acceptance-report "$evidence/commercial-chain.json" --subscription-acceptance-report "$evidence/subscription-chain.json" --provider-reconciliation-contract release/provider-contracts.json --quality-report "$evidence/quality-cases.json" --required-provider-grant-report "$evidence/provider-grant-fetch.json" --auth-origin-add-report "$evidence/staged-auth-origin-add.json" --auth-origin-remove-report "$evidence/staged-auth-origin-remove.json" --database-url-env PRODUCTION_MIGRATION_DATABASE_URL --approval-id-env PRODUCTION_ACCEPTANCE_APPROVAL_ID
```

The Supabase add operation first CAS-records the pre-change allowlist hash and exact intended callback; if its response is lost, cleanup reads back and reconciles rather than adding again. Production has no response-drop rule, fault intent, or tombstone lifecycle to create or clean. Failure cleanup keeps cohort flags OFF, disables Worker dispatch when required, removes the exact staged callback and DB origin, restores/verifies the pre-change allowlist hash, and requires no wildcard or staged residue.
